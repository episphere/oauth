        ...
        function tabulateParms(profile){
            ...
            Object.keys(profile).sort().forEach(k=>{
                if(k!="access_token"){
                    ...
                    h+=`<tr><td align="right" style="vertical-align:top"><b style="color:maroon">${k}</b>:</td><td><textarea id="bearer" rows=4 columns=30>${profile[k]}</textarea><button style="vertical-align:top" onclick="bearer.select();navigator.clipboard.writeText(bearer.value);setTimeout(_=>document.getSelection().removeAllRanges(),1000)">copy</button></td></tr>`
                }
